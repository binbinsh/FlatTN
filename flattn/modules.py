...
import math
import collections
from .module_utils import print_info, size2MB, MyDropout

def seq_len_to_mask(seq_len, max_len=None):
    if max_len is None:
        max_len = seq_len.max()
    rng = torch.arange(max_len, device=seq_len.device)
    return rng.unsqueeze(0) < seq_len.unsqueeze(1)
...
class Four_Pos_Fusion_Embedding(nn.Module):
    ...
    def forward(self,pos_s,pos_e):
        ...
        dev = pos_s.device
        pe_ss = self.pe_ss.to(dev)
        pe_se = self.pe_se.to(dev)
        pe_es = self.pe_es.to(dev)
        pe_ee = self.pe_ee.to(dev)
        pe_ss = pe_ss[(pos_ss).view(-1)+self.max_seq_len].view(size=[batch,max_seq_len,max_seq_len,-1])
        pe_se = pe_se[(pos_se).view(-1) + self.max_seq_len].view(size=[batch, max_seq_len, max_seq_len, -1])
        pe_es = pe_es[(pos_es).view(-1) + self.max_seq_len].view(size=[batch, max_seq_len, max_seq_len, -1])
        pe_ee = pe_ee[(pos_ee).view(-1) + self.max_seq_len].view(size=[batch, max_seq_len, max_seq_len, -1])
...
class Positionwise_FeedForward(nn.Module):
    def __init__(self, sizes, dropout=None,ff_activate='relu'):
        ...
        if dropout == None:
            dropout = collections.defaultdict(int)
        # ensure secondary FF dropout key exists; default to same as ff
        if 'ff_2' not in dropout:
            dropout['ff_2'] = dropout.get('ff', 0.0)
